...
- Annotation layers, each toggleable via the footer legend: lap or km split dots (`src/annotations.ts`, unit-tested; km marks thinned 1/2/5… per length), segment-effort halo spans (gold = PR, light purple = top-10), and grouped photo pins. The server resolves anchors to coordinate indices in `apps/server/src/mapAnchors.ts` (unit-tested) because Strava lap/effort indices reference the full-resolution stream, not the downsampled one. Annotation overlays render on the offline grid view only
- Optional MapLibre basemap (`src/BasemapView.tsx`, OpenFreeMap Liberty style) behind a "Basemap" pill, **off by default** so no-network-at-render stays the default; a failed style load bounces the toggle back to the offline grid. The track renders as GeoJSON line features reusing the same color binning (`buildColorRuns`; GeoJSON builders in `src/basemapData.ts`, unit-tested), with MapLibre's native zoom/pan behind `cooperativeGestures` (no scroll trap) and OSM attribution via MapLibre's control. The tile origin is allowlisted via `_meta.ui.csp` on the route-map resource (descriptor + content response, see `docs/plans/basemap-tile-source.md`). maplibre-gl is inlined by the single-file build: app.html ~658 KB → ~1.73 MB raw (~172 KB → ~447 KB gz). Basemap stories set `chromatic: { disableSnapshot: true }` (live tiles would diff every run)
