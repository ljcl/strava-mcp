...
const ExploreSegmentsInputSchema = z.object({
  ...
  activityType: z
    ...
    .optional()
    .describe(
      "Filter segments by activity type (optional: 'running' or 'riding').",
    ),
  minCat: z
    .number()
    .int()
    ...
    .optional()
    .describe(
      "Filter by minimum climb category (optional, 0-5). Requires riding activityType.",
    ),
  maxCat: z
    .number()
    .int()
    ...
    .optional()
    .describe(
      "Filter by maximum climb category (optional, 0-5). Requires riding activityType.",
    ),
});
  ...
  execute: async ({
    ...
    try {
      ...
      const elevationFactor =
        athlete.measurement_preference === "feet" ? 3.28084 : 1;
      const elevationUnit =
        athlete.measurement_preference === "feet" ? "ft" : "m";

      const segmentItems = response.segments.map((segment) => {
        const distance = (segment.distance * distanceFactor).toFixed(2);
        const elevDifference = (
          segment.elev_difference * elevationFactor
        ).toFixed(0);
        const text = `
