  ...
  execute: async ({ id }: GetActivityLapsInput) => {
    ...
    try {
      ...
      const [activity, laps] = await Promise.all([
        getActivityById(token, numericId),
      ...
      if (!laps || laps.length === 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: `✅ No laps found for activity ID: ${id}`,
            },
          ],
        };
      }

      // Generate human-readable summary with running-specific transformations
      const lapSummaries = laps.map((lap) => {
        const pace = metersPerSecToPace(lap.average_speed);
        const cadence = transformCadence(lap.average_cadence, activityType);
        ...
        const details = [
          ...
          // Show pace for running, speed for others
          isRunning && pace
            ? `  Pace: ${pace.minPerKm} /km (${pace.minPerMile} /mi)`
            : null,
      ...
      console.error(
        `Successfully fetched ${laps.length} laps for activity ${id}`,
      );
