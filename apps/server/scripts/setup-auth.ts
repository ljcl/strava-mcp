...
async function loadEnv(): Promise<{
  ...
  try {
    ...
    console.log(
      ".env file not found or not readable. Will prompt for all values.",
    );
    return {};
  }
}
...
async function main() {
  ...
  console.log("");
  console.log(
    "Tip: If running with Docker + Tailscale Funnel, visit /auth/start instead.",
  );
  console.log("");
  ...
  if (!clientSecret) {
    clientSecret = await promptUser(
      "Enter your Strava Application Client Secret: ",
    );
    if (!clientSecret) {
  ...
  console.log(`\n${authUrl}\n`);
  console.log(
    `After authorizing, Strava will redirect you to ${REDIRECT_URI}.`,
  );
  console.log(
  ...
  try {
    ...
    if (save.toLowerCase() === "yes" || save.toLowerCase() === "y") {
      ...
      try {
        ...
        /* Ignore if file doesn't exist, it was created in updateEnvFile */
      }
      ...
      if (!envContent.includes("STRAVA_CLIENT_SECRET=")) {
        ...
        if (tokenLineIndex !== -1) {
          envContent = `${envContent.substring(
            0,
            tokenLineIndex,
          )}STRAVA_CLIENT_SECRET=${clientSecret}\n${envContent.substring(
            tokenLineIndex,
          )}`;
        } else {
